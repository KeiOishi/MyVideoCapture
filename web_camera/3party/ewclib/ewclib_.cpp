...
void EWC_Cnv32to24(unsigned char *dst, unsigned int *src, int pxl)
{
	...
	for (int n = 0; n<pxl; n++){
		ui = *src++;
		B = static_cast<unsigned char>(ui);
		G = static_cast<unsigned char>(ui >> 8);
		R = static_cast<unsigned char>(ui >> 16);
		*(dst + 0) = B;
		*(dst + 1) = G;
		*(dst + 2) = R;
		dst += 3;
...
int EWC_Open(int num, int wx, int wy, double fps, int device, GUID mstype, GUID dev_mstype)
{
	if (num<0 || num >= EWC_NCAMMAX) return 1;
...
int EWC_Open(int num, int wx, int wy, double fps, char *devicename, GUID mstype, GUID dev_mstype)
{
	if (num<0 || num >= EWC_NCAMMAX) return 1;
...
int EWC_Open(int num, char *devicename, GUID mstype, GUID dev_mstype)
{
	if (num<0 || num >= EWC_NCAMMAX) return 1;
...
int EWC_SaveProperty(int num, char *filename)
{
	if (numCheck(num)) return 1;
...
int EWC_LoadProperty(int num, char *filename)
{
	if (numCheck(num)) return 1;
